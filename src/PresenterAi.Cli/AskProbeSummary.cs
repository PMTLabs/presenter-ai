...
/// <list type="bullet">
/// <item>en ×3, vi ×3 and cap24 ×3 each PASS (i)–(viii) and the reply check (turn-taking, owner decision 2026-09-24) and
...
{
    private static readonly string[] Criteria = ["i", "ii", "iii", "iv", "v", "vi", "vii", "viii", "reply"];
    ...
    internal static string Table(IReadOnlyList<RunResult> runs, Verdict verdict)
    {
        ...
        {
            $"{"run",-18} {"i",-4} {"ii",-4} {"iii",-4} {"iv",-4} {"v",-4} {"vi",-4} {"vii",-4} {"viii",-4} {"rply",-4} {"trunc",-6} {"prov",-5} {"latency",9} {"wire lag",9} {"overrun",8} {"usage s",8}  result",
            new string('-', 126)
        };
        foreach (var run in runs)
        {
            ...
            lines.Add(
                $"{run.Name,-18} {Cell("i"),-4} {Cell("ii"),-4} {Cell("iii"),-4} {Cell("iv"),-4} {Cell("v"),-4} {Cell("vi"),-4} {Cell("vii"),-4} {Cell("viii"),-4} {Cell("reply"),-4} " +
    ...
    [GeneratedRegex(@"^(?<verdict>PASS|FAIL) \((?<id>i|ii|iii|iv|v|vi|vii|viii|reply)\)", RegexOptions.Multiline)]
