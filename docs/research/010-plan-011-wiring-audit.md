...
- **Known input confirmed and widened.** 11 of 23 single-line gate edits leave every suite green. The 23 edits cover the 19 `ExchangeAllows`/`EmitOrDefer`
  call sites (the tool-command check included) and the 4 `ArmAnswerWait` routes. The survivors are
