...
| 2026-09-25 | Live fix: a repeated `revise_script` after an applied edit | T13 rows 3–5: the model re-delegated the confirmed edit after it applied; the cached `ok` made it announce the edit mid-replay and the question resume skipped the rest of the slide. A repeat now answers by the edit's state (unsettled: as before; applied: `already_done`, and the current slide is replayed at the question resume; failed: a failure result). The approval key includes the target slides. Impact analysis: docs/review/031. Mutation-checked (5 of 5). |
| 2026-09-25 | Live fix: the repeat's replay is gated on narration coverage | T13 row 5 on the 011 build with `889e61f` (slide 3): the repeat got `already_done`, the model stayed quiet and finished the slide, and the unconditional replay spoke it again (the first run, before `889e61f`, lost the rest of slide 2 instead). The voice stream has no completion event, so the repeat now sets its own flag, and at the resume (question resume, or `ResumeCore` after a Pause) `SettleRepeatReplay` skips the replay only when all parts were sent and the model's output transcript since the slide was presented covers it (`NarrationCoverage`: at least 90 % of the slide's words in order and its last six words together, Unicode-aware); otherwise, and when unsure, it replays. It never cancels `_replayOnResume`; the transcript and the flag reset on every `PresentSlide`, and the flag clears on wrap-up, Start/End and reconnect. Reviewed by pi (gpt-6-sol medium), `docs/review/032-plan-010-repeat-replay-coverage-gate.md`. Tests: `NarrationCoverageTests` (8) and 4 `PresenterTrainingTests` methods, 7 cases (spoken in full → one resume instruction, no replay; partial, quoted tail, or spoken only before the replay → one replay; Pause/Resume settles both ways; navigation leaves no replay); mutation-checked 6 of 6. |
