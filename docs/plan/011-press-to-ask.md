...
All 14 rows passed live on 2026-09-24 (row 4 after fixes) and again in the full regression on 2026-09-25 (row 8 after `94b2099`); see the work log `docs/progress/002-work-log-phase0.md`.
...
| 2026-09-24 | T8 regression fix: cut-off nudge at the cap | At `limit_sent` the burst's transcript ends mid-sentence (e.g. "…What did the program") and no answer came in 15 s (3/3 runs, one with speech ending exactly at the cap); a complete 19.4 s question by Ask done was answered in 8.0 s. Rule: only for `limit_sent`, 3 s after the send (`CutOffNudgeMs`) with the exchange still AwaitingAnswer, no answer voiced and no tool or backend work pending, `AskCutOffInstruction` (answer what was heard in the talk's language in one to three sentences, or ask to repeat) is appended once; each user delta while AwaitingAnswer keeps it at least 2 s away (`CutOffQuietMs`); an answer, follow-up or exchange end cancels it. Re-run: the descriptive nudge at +10.3 s (the burst transcript streamed for about 8 s) left the model silent and the 15 s budget ended the exchange 4.7 s later. The nudge now says "Answer it now … Then stop and wait." (the `ResumeAfterAskInstruction` pattern). Second re-run: the upstream took the cut-off 25 s burst in at about its own length (its last words transcribed at +23.7 s; the reply "didn't catch the full question, could you repeat" at +24.6 s) and the budget re-armed from the nudge resumed the talk over it at +25.4 s. A `limit_sent` burst now waits `AnswerStartBudgetMs` + its kept speech (40 s at the cap), and the nudge only extends the running budget to at least 15 s from the nudge, never shortens it; all within the ceiling |
| 2026-09-24 | T8 regression fix: answer-now nudge for complete questions | Row 2 on `b1bb46d`: the delegated `go_to_slide` call of the first question arrived 0.5 s into the follow-up Ask and was refused busy (site 2, by design, no `response.create`); the model then left that question and the next, direct one (`en-lookup`) unanswered for 15 s each, while it still obeyed the resume instructions. The same refusal was answered normally in two other runs, so it is intermittent upstream behaviour, not a missing continue. Rule: every non-`limit_sent` send arms the same one-shot nudge 6 s after the send (`AnswerNudgeMs`); each user delta of its transcript keeps it at least 3 s away (`AnswerNudgeQuietMs`; live answers start 1.3–3 s after the send, a 19 s question at 8 s). It is skipped once answer audio, a pending tool or a backend delegation exists, appends `PromptBuilder.AskAnswerNowInstruction()` ("Answer it now … Then stop and wait", no quoted phrase) once as `{exchange}-answer-now`, logs `ask: no answer yet; asked the model to answer the question now`, and leaves the 15 s budget unchanged. Tests: `Complete_question_without_an_answer_gets_one_answer_now_nudge_and_keeps_its_budget`, `Question_transcript_deltas_postpone_the_answer_now_nudge`, `Answer_or_answer_work_before_the_answer_now_nudge_cancels_it`, `Answer_now_nudge_answers_in_the_language_of_the_talk_without_a_phrase_to_read_out`; mutation-checked (limit-only arm, 2 s quiet, no delegation guard, budget extension). |
| 2026-09-24 | T8 regression fix: the Ask's own pause and resume wording | On `c6cca0c`, after three or four Asks the model left plain speech unanswered (4 of 4 spoken edit requests in two sessions: 15 s silence, no delegation), while a fresh session with no Ask delegated the same clip to `revise_script` at once. Cause (inferred, not proven): `PauseInstruction()` ("Stay silent … you may answer in a few words") appended at every Ask. Rule: the Ask start appends `PromptBuilder.AskPauseInstruction()` ("Pause now: a listener is asking a question. Stay silent until you hear it, then answer it. Do not continue the narration until told.", same `pause-N` id); manual Pause and the other `PauseCore` callers keep the Node golden `PauseInstruction()`. `ResumeAfterAskInstruction` adds "If someone speaks to you afterwards, answer them or pass on their request as usual." before "Then stop and wait." Tests: `Ask_during_narration_mutes_pauses_and_flushes_and_forwards_no_mic_audio` (content; mutation-checked), `Ask_pause_announces_the_question_and_asks_for_the_answer`, `Ask_resume_names_the_slide_ends_the_pause_and_keeps_the_transition`. Live check pending (handoff 033). Row 8's "One moment." taken as the answer was a harness artifact (the harness said "yes" 0.3 s into the check-in; the late delegation came 3.4 s in, inside the window the existing filler rule covers); no code change. |
| 2026-09-25 | T8 regression fixes: nudge after answer work; Responses channel errors | Live on `c132b68`: after four Asks, spoken edit requests were delegated again (2 of 2, both `revise_script … waiting for yes`; 0 of 4 before), so the pause fix holds. Two more defects. (1) The answer-now nudge fired 0.2 s after "backend answer ready", just before the delegated answer: a delegation event that the exchange accepts now stops the nudge (`StopAskCutOff` in the delegation handler), even after the backend has finished (`Answer_or_answer_work_before_the_answer_now_nudge_cancels_it("finished delegation")`). (2) The backend failed as `invalid_request_error` "Responses websocket closed before a terminal event."; only `backend_error` ended a pending delegation, so the re-delegated answer counted as filler and the exchange waited until the ceiling. Errors whose message starts with "Responses " now end it too; other errors (e.g. `rate_limit_exceeded`) still leave it pending (`Top_level_backend_error_ends_the_pending_backend_answer`, two cases). Both mutation-checked. The upstream created delegations 4.5–5.7 s after "One moment." in three asks, inside the check-in window the filler rule covers; the harness now answers a check-in only after more than a short filler was said. |
| 2026-09-25 | Interaction check: repeated `revise_script` (plan 010 `889e61f`) inside an exchange | Ask start clears the approval cache, so a repeat inside an exchange is always of an edit confirmed in it. That edit holds the current slide, and its apply releases the hold, which sets the exchange's `ReplayDue` (site 10). The repeat's own `_replayOnResume` is then cleared by that replay (`PresentSlide`) or by navigation, so the exchange keeps sole ownership of the replay. A guard (`_exchange is null`) planned in handoff 035 was dropped: it had no observable effect (mutant indistinguishable). Pinned instead by `PresenterAskTests.Repeated_request_for_an_edit_applied_during_the_exchange_replays_the_slide_once_after_it` (`already_done`, one replay with the new text, and the next Ask resumes without a replay); mutation-checked 2 of 2 (no repeat answer; no flag reset on present). No production change. |
| 2026-09-25 | Site 10: the repeat's replay is settled at Ask start (plan 010 coverage gate) | Plan 010 now gates the repeated-request replay on narration coverage (`_repeatReplayDue`, `SettleRepeatReplay`; plan 010 §10, same date). Ask start calls `SettleRepeatReplay` before the new exchange copies `_replayOnResume` into `ReplayDue`, so an unfinished slide is replayed once when the exchange ends ("replay yes") and a slide spoken in full gets the Ask resume ("replay no"), instead of the open flag being decided after the exchange by the question resume. A repeat inside an exchange still needs no flag of its own: the exchange's `ReplayDue` replay clears it (row above). Test: `PresenterAskTests.Ask_during_a_repeated_request_settles_its_replay_before_the_exchange_takes_it_over` (both cases); mutation-checked (no settle at Ask start fails both). |
